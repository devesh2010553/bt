...
app.post('/api/auth/firebase-login', async (req, res) => {
  ...
  if (!firebaseReady) return res.status(503).json({ error: 'Firebase not configured on server' });
  try {
    const decoded = await admin.auth().verifyIdToken(idToken);
    const name = decoded.name || decoded.email.split('@')[0];
    const email = decoded.email;
    req.session.customer = { email, name, uid: decoded.uid, provider: 'google' };
    res.json({ success: true, name, email });
  } catch(e) {
    console.log('Firebase token verify error:', e.message);
...
app.post('/api/driver/reset-secret/send-otp', async (req, res) => {
  ...
  sendEmail(email, `Brajwasi Driver Secret Reset OTP: ${otp}`,
    `<div style="font-family:Arial,sans-serif;max-width:420px;padding:24px;background:#fff;border-radius:12px">
      ...
      <div style="font-size:36px;font-weight:900;color:#B8780A;letter-spacing:10px;text-align:center;padding:20px;background:#FFF8EC;border-radius:12px;margin:16px 0">${otp}</div>
      <p style="color:#7a6a55;font-size:13px">Expires in 10 minutes. Do not share with anyone.</p>
    </div>`);
  res.json({ success: true });
});
