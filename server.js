...
// ── Push helpers ─────────────────────────────────────────────
const sendPush = async (subs, title, body, data = {}) => {
...
// Send only to admin devices
const pushAdmin = async (title, body, data = {}) => {
  try {
    const subs = await PushSubscription.find({ role: 'admin' });
    await sendPush(subs, title, body, data);
  } catch (e) { console.log('Push admin error:', e.message); }
};

// Send only to a specific customer by phone
const pushCustomer = async (phone, title, body, data = {}) => {
  if (!phone) return;
  try {
    const subs = await PushSubscription.find({ role: 'customer', phone: phone.trim() });
    await sendPush(subs, title, body, data);
  } catch (e) { console.log('Push customer error:', e.message); }
};

// Send to admin + a specific customer (for booking confirmations)
const pushAdminAndCustomer = async (phone, title, body, data = {}) => {
  try {
    const subs = await PushSubscription.find({
      $or: [{ role: 'admin' }, { role: 'customer', phone: phone?.trim() || '__none__' }]
    });
    await sendPush(subs, title, body, data);
...
app.post('/api/book', async (req, res) => {
  try {
    ...
    await pushAdmin('🚗 New Booking!',
      `${customerName} · ${car.model} · ${pickupLocation} → ${dropLocation}`,
      { url: '/admin/bookings', bookingId });

    // Notify the customer on their own device only (booking receipt)
    await pushCustomer(customerPhone,
      '🎉 Booking Received!',
      `Hi ${customerName}! Your booking ${bookingId} is received. We will confirm shortly.`,
      { url: '/track?bookingId=' + bookingId, bookingId });
...
app.post('/admin/bookings/:id/status', adminAuth, async (req, res) => {
  ...
  if (req.body.status === 'confirmed') {
    // Push ONLY to that specific customer's device (matched by their phone number)
    await pushCustomer(
      booking.customerPhone,
      '✅ Booking Confirmed!',
      `Hi ${booking.customerName}! Your booking ${booking.bookingId} is confirmed. We'll be there on time.`,
    ...
    await pushCustomer(
      booking.customerPhone,
      '❌ Booking Cancelled',
      `Your booking ${booking.bookingId} has been cancelled. Please call 9411061000 for help.`,
...
// Push (manual broadcast — admin only, not customers)
...
app.post('/admin/chat/:sessionId/reply', adminAuth, async (req, res) => {
  ...
  // Notify ONLY that specific customer
  await pushCustomer(
